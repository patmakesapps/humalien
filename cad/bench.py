...
B = dict(
    ...
    tab_x=30.0,             # was 55: the gantry legs now own that lane, and
                            # a tab screw needs a screwdriver behind it
...
STATIONS = dict(
    ...
    # SHT41: mounting deferred - the board on hand is not the Adafruit
    # breakout the 20.32 x 12.7 guess assumed. It ties down in the cable
    # channel until its real hole pattern is measured; behind the PCA,
    # away from the Pi's heat, is still where it should live.
)
...
def printcheck():
    ns = _checks()
    return ns["gate"]([(n, ns[o], s) for n, o, s, _c in PLATE])
...
def export_stls(dirpath=r"C:\Humalien\exports\bench"):
    ...
    out = []
    for name, okey, _sup, count in PLATE:
        ...
        path = os.path.join(dirpath, "%s_x%d.stl" % (name.lower(), count))
